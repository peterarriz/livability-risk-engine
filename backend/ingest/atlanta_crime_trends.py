"""
backend/ingest/atlanta_crime_trends.py
task: data-056
lane: data

Ingests Atlanta Police Department crime data and calculates 12-month
crime trends by zone.

Source:
  ArcGIS FeatureServer — APD Crime Data
  https://services3.arcgis.com/Et5Qfajgiyosiw4d/arcgis/rest/services/OpenDataWebsite_Crime_view/FeatureServer/0

  Key fields: OccurredFromDate, Zone, NIBRS_Offense

Output:
  data/raw/atlanta_crime_trends.json

Usage:
  python backend/ingest/atlanta_crime_trends.py
  python backend/ingest/atlanta_crime_trends.py --dry-run
"""
...
DEFAULT_OUTPUT_PATH = Path("data/raw/atlanta_crime_trends.json")
...
def fetch_crime_counts(
    ...
    for feature in payload.get("features", []):
        ...
        if not zone:
            continue
        results[zone] = results.get(zone, 0) + int(attrs.get("crime_count") or attrs.get("CRIME_COUNT") or 0)
    return results
...
def build_trend_records(
    ...
    for zone in sorted(all_zones):
        ...
        records.append({
            "region_type": "zone",
            "region_id": f"atlanta_zone_{slug}",
            "district_id": zone,
            "district_name": f"Atlanta {zone}",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest Atlanta APD crime trends by zone from ArcGIS FeatureServer."
    )
...
def main() -> None:
    ...
    print(f"Fetching current 12-month Atlanta crime counts ({current_start:%Y-%m-%d} → {now:%Y-%m-%d})...")
    try:
    ...
    print(f"\nFetching prior 12-month Atlanta crime counts ({prior_start:%Y-%m-%d} → {prior_end:%Y-%m-%d})...")
    try:
