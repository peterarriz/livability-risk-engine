...
Ingests Irving Police Department crime data and calculates 12-month
crime trends by beat via ArcGIS REST API.

Source:
  Irving Open Data — IPD Crime Incidents:
  https://cityofirving.org/299/Open-Data
  https://services.arcgis.com/9xyBGNHCPT1TXqR6/arcgis/rest/services
      /IPD_Crime_Incidents/FeatureServer/0
  ...
  Verify endpoint and field names:
    curl "https://services.arcgis.com/9xyBGNHCPT1TXqR6/arcgis/rest/services/IPD_Crime_Incidents/FeatureServer/0/query?where=1%3D1&outFields=*&resultRecordCount=1&f=json"

  Key fields (MUST VERIFY):
    IncidentDate  — date field
    Beat          — police beat (may be "Sector" or "District")
    Latitude      — double
    Longitude     — double
...
Output:
  data/raw/irving_tx_crime_trends.json — beat crime trend records

Usage:
  python backend/ingest/irving_tx_crime_trends.py
  python backend/ingest/irving_tx_crime_trends.py --dry-run
"""
...
ARCGIS_BASE_URL = (
    "https://services.arcgis.com/9xyBGNHCPT1TXqR6/arcgis/rest/services"
    "/IPD_Crime_Incidents/FeatureServer/0/query"
)

DEFAULT_OUTPUT_PATH = Path("data/raw/irving_tx_crime_trends.json")

DATE_FIELD = "IncidentDate"   # MUST VERIFY
DISTRICT_FIELD = "Beat"       # MUST VERIFY — may be "Sector" or "District"
...
def build_trend_records(
    ...
    for district in sorted(all_districts):
        ...
        records.append({
            "region_type": "beat",
            "region_id": f"irving_beat_{district}",
            "district_id": district,
            "district_name": f"Irving Beat {district}",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest Irving PD crime trends by beat from ArcGIS REST API."
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def main() -> None:
    ...
    print(f"Fetching current 12-month Irving crime counts ({_timestamp_str(current_start)} → {_timestamp_str(now)})...")
    try:
    ...
    print(f"\nFetching prior 12-month Irving crime counts ({_timestamp_str(prior_start)} → {_timestamp_str(prior_end)})...")
    try:
