"""
backend/ingest/cleveland_crime_trends.py
task: data-056
lane: data

Ingests Cleveland Police Department crime data and calculates 12-month
crime trends by district.

Source:
  ArcGIS FeatureServer — Crime Incidents
  https://services3.arcgis.com/dty2kHktVXHrqO8i/arcgis/rest/services/Crime_Incidents/FeatureServer/0

  Key fields: OffenseDate, District, UCRdesc

Output:
  data/raw/cleveland_crime_trends.json

Usage:
  python backend/ingest/cleveland_crime_trends.py
  python backend/ingest/cleveland_crime_trends.py --dry-run
"""
...
DEFAULT_OUTPUT_PATH = Path("data/raw/cleveland_crime_trends.json")
...
def fetch_crime_counts(
    ...
    for feature in payload.get("features", []):
        ...
        if not district:
            continue
        results[district] = results.get(district, 0) + int(attrs.get("crime_count") or attrs.get("CRIME_COUNT") or 0)
    return results
...
def build_trend_records(
    ...
    for district in sorted(all_districts):
        ...
        records.append({
            ...
            "region_id": f"cleveland_district_{slug}",
            "district_id": district,
            "district_name": f"Cleveland {district}",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest Cleveland CPD crime trends by district from ArcGIS FeatureServer."
    )
...
def main() -> None:
    ...
    print(f"Fetching current 12-month Cleveland crime counts ({current_start:%Y-%m-%d} → {now:%Y-%m-%d})...")
    try:
    ...
    print(f"\nFetching prior 12-month Cleveland crime counts ({prior_start:%Y-%m-%d} → {prior_end:%Y-%m-%d})...")
    try:
