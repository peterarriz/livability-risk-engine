...
Ingests Murfreesboro Police Department crime data and calculates 12-month
crime trends by zone via ArcGIS REST API.

Source:
  Murfreesboro Open Data — MPD Crime Incidents:
  https://murfreesborotn.gov/opendata
  https://services1.arcgis.com/QpJd9AijpBIH7O5B/arcgis/rest/services
      /MPD_Crime_Incidents/FeatureServer/0
  ...
  Verify service listing:
    curl -s "https://services1.arcgis.com/QpJd9AijpBIH7O5B/arcgis/rest/services?f=json" \
      | python3 -c "import sys,json; [print(s['name']) for s in json.load(sys.stdin).get('services',[])]"

  Verify endpoint and field names:
    curl "https://services1.arcgis.com/QpJd9AijpBIH7O5B/arcgis/rest/services/MPD_Crime_Incidents/FeatureServer/0/query?where=1%3D1&outFields=*&resultRecordCount=1&f=json"

  Key fields (MUST VERIFY):
    IncidentDate  — date field
    Zone          — police zone (may be "Sector" or "Precinct")
    Latitude      — double
    Longitude     — double
    ObjectId      — used for count()

Method:
  1. Aggregate crime counts + avg lat/lon by zone for the last 12 months
     via outStatistics.
  2. Aggregate crime counts + avg lat/lon by zone for the prior 12 months.
  3. Calculate percent change → crime_trend: INCREASING / DECREASING / STABLE.

Output:
  data/raw/murfreesboro_crime_trends.json — zone crime trend records

Usage:
  python backend/ingest/murfreesboro_crime_trends.py
  python backend/ingest/murfreesboro_crime_trends.py --dry-run
"""
...
ARCGIS_BASE_URL = (
    "https://services1.arcgis.com/QpJd9AijpBIH7O5B/arcgis/rest/services"
    "/MPD_Crime_Incidents/FeatureServer/0/query"
)

DEFAULT_OUTPUT_PATH = Path("data/raw/murfreesboro_crime_trends.json")

DATE_FIELD = "IncidentDate"   # MUST VERIFY
DISTRICT_FIELD = "Zone"       # MUST VERIFY — may be "Sector" or "Precinct"
...
MURFREESBORO_LAT = 35.8456
MURFREESBORO_LON = -86.3903
...
def fetch_crime_counts_with_centroids(
    ...
    """
    Fetch total crime counts and average centroids per zone for a date range.
...
def build_trend_records(
    ...
    All zones appearing in either window get a record.
    """
    ...
    for district in sorted(all_districts):
        ...
        records.append({
            "region_type": "zone",
            "region_id": f"murfreesboro_zone_{district}",
            "district_id": district,
            "district_name": f"Murfreesboro Zone {district}",
...
def write_staging_file(records: list[dict], output_path: Path) -> None:
    ...
    staging = {
        "source": "murfreesboro_crime_trends",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest Murfreesboro PD crime trends by zone from ArcGIS REST API."
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def main() -> None:
    ...
    print(f"Fetching current 12-month Murfreesboro crime counts ({_timestamp_str(current_start)} → {_timestamp_str(now)})...")
    try:
    ...
    print(f"\nFetching prior 12-month Murfreesboro crime counts ({_timestamp_str(prior_start)} → {_timestamp_str(prior_end)})...")
    try:
