"""
backend/ingest/wichita_crime_trends.py
task: data-057
lane: data

Ingests Wichita Police Department crime data and calculates 12-month crime
...
Usage:
  python backend/ingest/wichita_crime_trends.py
  python backend/ingest/wichita_crime_trends.py --dry-run
"""
...
def build_trend_records(
    ...
    for district in sorted(all_districts):
        ...
        records.append({
            ...
            "region_id": f"wichita_district_{district}",
            "district_id": district,
            "district_name": f"Wichita District {district}",
            "crime_12mo": current_count,
            "crime_prior_12mo": prior_count,
            "crime_trend": trend,
            "crime_trend_pct": trend_pct,
            "latitude": WICHITA_LAT,
            "longitude": WICHITA_LON,
        })
    return records
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest Wichita PD crime trends by district from ArcGIS."
    )
...
def main() -> None:
    ...
    print("Fetching current 12-month Wichita crime counts...")
    try:
        current_data = fetch_crime_counts(current_start, now)
    except Exception as exc:
        print(f"ERROR: failed to fetch current Wichita crime counts — {exc}", file=sys.stderr)
        sys.exit(1)
    print(f"  {len(current_data)} districts with current crime data.")

    print("Fetching prior 12-month Wichita crime counts...")
    try:
        prior_data = fetch_crime_counts(prior_start, prior_end)
    except Exception as exc:
        print(f"ERROR: failed to fetch prior Wichita crime counts — {exc}", file=sys.stderr)
        sys.exit(1)
