"""
backend/ingest/chattanooga_crime_trends.py
task: data-059
lane: data

Ingests Chattanooga Police Department crime data and calculates
12-month crime trends by incident type.

Source:
  ArcGIS FeatureServer — Chattanooga Open Data (ChattaData)
  Portal: https://www.chattadata.org
  Service: Police Incident Data FeatureServer/0
...
Usage:
  python backend/ingest/chattanooga_crime_trends.py
...
FEATURESERVER_URL = (
    "https://services2.arcgis.com/OIAIimblRxPs0xxc/arcgis/rest/services"
    "/testingtestingtestingpolicepoints/FeatureServer/0"
)
...
def build_trend_records(
    ...
    for district in sorted(all_districts):
        ...
        records.append({
            ...
            "region_id": f"chattanooga_type_{slug}",
            "district_id": district,
            "district_name": f"Chattanooga {district}",
            "crime_12mo": current_count,
            "crime_prior_12mo": prior_count,
            "crime_trend": trend,
            "crime_trend_pct": trend_pct,
            "latitude": CHATTANOOGA_LAT,
            "longitude": CHATTANOOGA_LON,
        })
    return records
...
def write_staging_file(records: list[dict], output_path: Path) -> None:
    ...
    staging = {
        "source": "chattanooga_crime_trends",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest Chattanooga CPD crime trends by type from ArcGIS FeatureServer."
    )
...
def main() -> None:
    ...
    print(f"Chattanooga crime trends ingest — source: {FEATURESERVER_URL}")
    ...
    print(f"Fetching current 12-month Chattanooga crime counts ({current_start:%Y-%m-%d} → {now:%Y-%m-%d})...")
    try:
    ...
    print(f"  {len(current_data)} types, {total_current:,} total crimes.")

    print(f"\nFetching prior 12-month Chattanooga crime counts ({prior_start:%Y-%m-%d} → {prior_end:%Y-%m-%d})...")
    try:
    ...
    print(f"  {len(prior_data)} types, {total_prior:,} total crimes.")

    records = build_trend_records(current_data, prior_data)
    print(f"\nBuilt {len(records)} type trend records.")
