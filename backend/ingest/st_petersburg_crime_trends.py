...
Ingests St. Petersburg Police Department crime data and calculates 12-month
crime trends by sector via ArcGIS REST API.

Source:
  St. Petersburg Open Data — SPPD Crime Incidents:
  https://data.stpete.org
  ...
  Verify endpoint and field names:
    curl "https://services1.arcgis.com/8vEm1j5dMMr9eBob/arcgis/rest/services/SPPD_Crime_Incidents/FeatureServer/0/query?where=1%3D1&outFields=*&resultRecordCount=1&f=json"

  Key fields (MUST VERIFY):
    IncidentDate  — date field
    Sector        — police sector (may be "Zone" or "Beat")
    Latitude      — double
    Longitude     — double
...
Output:
  data/raw/st_petersburg_crime_trends.json — sector crime trend records

Usage:
  python backend/ingest/st_petersburg_crime_trends.py
  python backend/ingest/st_petersburg_crime_trends.py --dry-run
"""
...
DEFAULT_OUTPUT_PATH = Path("data/raw/st_petersburg_crime_trends.json")

DATE_FIELD = "IncidentDate"   # MUST VERIFY
DISTRICT_FIELD = "Sector"     # MUST VERIFY — may be "Zone" or "Beat"
...
def build_trend_records(
    ...
    for district in sorted(all_districts):
        ...
        records.append({
            "region_type": "sector",
            "region_id": f"st_pete_sector_{district}",
            "district_id": district,
            "district_name": f"St. Petersburg Sector {district}",
...
def write_staging_file(records: list[dict], output_path: Path) -> None:
    ...
    staging = {
        "source": "st_petersburg_crime_trends",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest St. Petersburg PD crime trends by sector from ArcGIS REST API."
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def main() -> None:
    ...
    print(f"Fetching current 12-month St. Petersburg crime counts ({_timestamp_str(current_start)} → {_timestamp_str(now)})...")
    try:
    ...
    print(f"\nFetching prior 12-month St. Petersburg crime counts ({_timestamp_str(prior_start)} → {_timestamp_str(prior_end)})...")
    try:
