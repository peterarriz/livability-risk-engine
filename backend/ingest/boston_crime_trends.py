...
Ingests Boston Police Department crime data and calculates 12-month crime trends
by district.
...
Source:
  ...
  Sample first record to confirm field names:
    curl "https://data.boston.gov/api/3/action/datastore_search?resource_id=<UUID>&limit=1"

Method:
  1. Use CKAN datastore_search_sql to aggregate crime counts by district.
  2. Fall back to plain datastore_search with client-side filtering if SQL endpoint
     returns 403/501.
  3. Calculate percent change → crime_trend: INCREASING / DECREASING / STABLE.

Output:
  data/raw/boston_crime_trends.json — district crime trend records
...
def build_trend_records(
    ...
    Note: CKAN aggregation does not provide lat/lon centroids; coordinates
    are left as None for Boston crime trends.
    """
    ...
    for district in sorted(all_districts):
        ...
        records.append({
            ...
            "region_id": f"boston_district_{district}",
...
def discover_resource() -> None:
    ...
    params = {"q": "crime incident", "rows": 10}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest Boston BPD crime trends by district from CKAN."
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def main() -> None:
    ...
    try:
        ...
        print(
            ...
            f"  python backend/ingest/boston_crime_trends.py --discover",
            file=sys.stderr,
        )
        sys.exit(1)
