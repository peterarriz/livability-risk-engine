...
Source:
  ...
  Key fields (MUST VERIFY via sample query):
    ...
    District      — SJPD district
    Latitude, Longitude — coordinates

Output:
  data/raw/san_jose_crime_trends.json — district crime trend records
...
# MUST VERIFY: visit https://gis.sanjoseca.gov, search "crime" or "police incidents",
# open the dataset, click "I want to use this" → "API Explorer" → copy FeatureServer URL.
FEATURESERVER_URL = (
    "https://services.arcgis.com/p8Tul9YqBFRRdPqD/arcgis/rest/services"
    "/SJPD_Crime/FeatureServer/0"
)
PORTAL_ORG_ID = "p8Tul9YqBFRRdPqD"
...
def build_trend_records(
    ...
    for district in sorted(all_districts):
        ...
        records.append({
            ...
            "region_id": f"san_jose_district_{slug}",
            "district_id": district,
            "district_name": f"San Jose SJPD District {district}",
...
def discover(org_id: str) -> None:
    ...
    print(f"\nAlso try: https://gis.sanjoseca.gov (search 'crime' or 'police')")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest San Jose SJPD crime trends by district from ArcGIS FeatureServer."
    )
