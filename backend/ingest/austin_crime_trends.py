"""
backend/ingest/austin_crime_trends.py
task: data-044
lane: data
...
Method:
  ...
  2. Aggregate crime counts by sector for the prior 12 months (baseline).
  3. Calculate percent change → crime_trend: INCREASING / DECREASING / STABLE.
  4. Compute approximate sector centroids from the average lat/lon of incidents.

Output:
  data/raw/austin_crime_trends.json — sector crime trend records

Usage:
  python backend/ingest/austin_crime_trends.py
  python backend/ingest/austin_crime_trends.py --dry-run
...
def fetch_crime_counts_with_centroids(
    ...
    for row in rows:
        sector = str(row.get(DISTRICT_FIELD, "") or "").strip().upper()
        if not sector:
            continue
        try:
...
def build_trend_records(
    ...
    for sector in sorted(all_sectors):
        curr = current_data.get(sector, {"count": 0, "lat": None, "lon": None})
        prev = prior_data.get(sector, {"count": 0, "lat": None, "lon": None})
        ...
        records.append({
            "region_type": "sector",
            "region_id": f"austin_sector_{sector}",
            "district_id": sector,
            "district_name": f"Austin Sector {sector}",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest Austin APD crime trends by sector from the Socrata API."
    )
    parser.add_argument(
        "--output",
        type=Path,
