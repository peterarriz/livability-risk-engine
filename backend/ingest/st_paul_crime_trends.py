...
Ingests St. Paul Police Department crime data and calculates 12-month
crime trends by precinct via ArcGIS REST API.

Source:
  St. Paul Open Data — SPPD Crime Incidents:
  https://information.stpaul.gov
  https://services.arcgis.com/v400IkDOw1ad7Yad/arcgis/rest/services
      /SPPD_Crime_Incidents/FeatureServer/0
  ...
  Verify service listing:
    curl -s "https://services.arcgis.com/v400IkDOw1ad7Yad/arcgis/rest/services?f=json" \
      | python3 -c "import sys,json; [print(s['name']) for s in json.load(sys.stdin).get('services',[])]"

  Verify endpoint and field names:
    curl "https://services.arcgis.com/v400IkDOw1ad7Yad/arcgis/rest/services/SPPD_Crime_Incidents/FeatureServer/0/query?where=1%3D1&outFields=*&resultRecordCount=1&f=json"

  Key fields (MUST VERIFY):
    DateOccurred  — date field
...
Method:
  1. Aggregate crime counts + avg lat/lon by precinct for the last 12 months
     via outStatistics.
  2. Aggregate crime counts + avg lat/lon by precinct for the prior 12 months.
  3. Calculate percent change → crime_trend: INCREASING / DECREASING / STABLE.

Output:
  data/raw/st_paul_crime_trends.json — precinct crime trend records

Usage:
  python backend/ingest/st_paul_crime_trends.py
  python backend/ingest/st_paul_crime_trends.py --dry-run
"""
...
ARCGIS_BASE_URL = (
    "https://services.arcgis.com/v400IkDOw1ad7Yad/arcgis/rest/services"
    "/SPPD_Crime_Incidents/FeatureServer/0/query"
)

DEFAULT_OUTPUT_PATH = Path("data/raw/st_paul_crime_trends.json")

DATE_FIELD = "DateOccurred"   # MUST VERIFY
DISTRICT_FIELD = "Precinct"   # MUST VERIFY
...
def build_trend_records(
    ...
    for district in sorted(all_districts):
        ...
        records.append({
            ...
            "region_id": f"st_paul_precinct_{district}",
            "district_id": district,
            "district_name": f"St. Paul Precinct {district}",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest St. Paul PD crime trends by precinct from ArcGIS REST API."
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def main() -> None:
    ...
    print(f"Fetching current 12-month St. Paul crime counts ({_timestamp_str(current_start)} → {_timestamp_str(now)})...")
    try:
    ...
    print(f"\nFetching prior 12-month St. Paul crime counts ({_timestamp_str(prior_start)} → {_timestamp_str(prior_end)})...")
    try:
