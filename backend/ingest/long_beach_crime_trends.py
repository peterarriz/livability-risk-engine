...
Ingests Long Beach LBPD crime data and calculates 12-month crime trends by area.

Source:
  Socrata — data.longbeach.gov
  Dataset: LBPD Crime Data (not live-verified via catalog API)
  Dataset ID: 4bz9-ggsz (MUST VERIFY — may be different)

  Verify dataset ID:
    curl "https://data.longbeach.gov/api/catalog/v1?q=crime+incidents&limit=10"

  Verify sample row / field names:
    curl "https://data.longbeach.gov/resource/4bz9-ggsz.json?$limit=1"

  Key fields (MUST VERIFY — names below are best-guess, not live-confirmed):
    date_rptd    — date of incident  (MUST VERIFY field name)
    area_name    — patrol area       (MUST VERIFY — may be "beat" or
                                      "reporting_district")
    lat          — latitude          (MUST VERIFY)
    lon          — longitude         (MUST VERIFY)
...
Method:
  1. Aggregate crime counts by area for the last 12 months (current window).
  2. Aggregate crime counts by area for the prior 12 months (baseline).
  3. Calculate percent change → crime_trend: INCREASING / DECREASING / STABLE.
  4. Compute approximate area centroids from the average lat/lon of incidents.

Output:
  data/raw/long_beach_crime_trends.json — area crime trend records
...
SOCRATA_DOMAIN = "data.longbeach.gov"
DATASET_ID = "4bz9-ggsz"   # MUST VERIFY — not live-confirmed
CRIMES_URL = f"https://{SOCRATA_DOMAIN}/resource/{DATASET_ID}.json"

DEFAULT_OUTPUT_PATH = Path("data/raw/long_beach_crime_trends.json")

# Date field and district field in the LBPD crime dataset.
# MUST VERIFY field names against the actual dataset before production use.
DATE_FIELD = "date_rptd"        # MUST VERIFY — may differ
DISTRICT_FIELD = "area_name"    # MUST VERIFY — may be "beat" or "reporting_district"
LAT_FIELD = "lat"               # MUST VERIFY
LON_FIELD = "lon"               # MUST VERIFY

# City centre fallback (unused in computation; reserved for future use)
LONG_BEACH_LAT = 33.7701
LONG_BEACH_LON = -118.1937
...
def fetch_crime_counts_with_centroids(
    ...
    """
    Fetch total crime counts per area for a date range, with avg lat/lon.
    Uses SoQL GROUP BY to compute aggregates server-side.

    Returns dict: area → {count, lat, lon}.

    MUST VERIFY: DATE_FIELD, DISTRICT_FIELD, LAT_FIELD, LON_FIELD, and
    DATASET_ID against data.longbeach.gov before production use.
    ...
    for row in rows:
        ...
        try:
            ...
            lon = None
        results[area] = {"count": count, "lat": lat, "lon": lon}
...
def build_trend_records(
    ...
    for area in sorted(all_areas):
        ...
        records.append({
            "region_type": "area",
            "region_id": f"long_beach_area_{area}",
            "district_id": area,
            "district_name": f"Long Beach Area {area}",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest Long Beach LBPD crime trends by area from the Socrata API."
    )
    parser.add_argument(
        "--output",
        type=Path,
