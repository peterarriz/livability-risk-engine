...
def main() -> None:
    ...
    print("Fetching current 12-month St. Louis crime counts...")
    try:
        current_data = fetch_crime_counts_with_centroids(app_token, current_start, now)
    except Exception as exc:
        print(f"ERROR: failed to fetch current St. Louis crime counts — {exc}", file=sys.stderr)
        sys.exit(1)
    print(f"  {len(current_data)} districts with current crime data.")

    print("Fetching prior 12-month St. Louis crime counts...")
    try:
        prior_data = fetch_crime_counts_with_centroids(app_token, prior_start, prior_end)
    except Exception as exc:
        print(f"ERROR: failed to fetch prior St. Louis crime counts — {exc}", file=sys.stderr)
        sys.exit(1)
