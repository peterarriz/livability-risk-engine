"""
backend/ingest/indianapolis_crime_trends.py
task: data-050
lane: data

Ingests Indianapolis Metropolitan Police Department (IMPD) crime data and
...
Usage:
  python backend/ingest/indianapolis_crime_trends.py
...
def build_trend_records(
    ...
    for district in sorted(all_districts):
        ...
        records.append({
            ...
            "region_id": f"indianapolis_district_{slug}",
            "district_id": district,
            "district_name": f"Indianapolis IMPD District {district}",
            "crime_12mo": current_count,
            "crime_prior_12mo": prior_count,
            "crime_trend": trend,
            "crime_trend_pct": trend_pct,
            "latitude": INDIANAPOLIS_LAT,
            "longitude": INDIANAPOLIS_LON,
        })
    return records
...
def write_staging_file(records: list[dict], output_path: Path) -> None:
    ...
    staging = {
        "source": "indianapolis_crime_trends",
...
def discover(org_id: str) -> None:
    ...
    try:
        ...
        resp.raise_for_status()
        data = resp.json()
        print("ArcGIS Hub search results (IMPD crime/incidents):")
        for item in data.get("data", []):
            attrs = item.get("attributes", {})
            print(f"  {attrs.get('name','?')} — {attrs.get('url','?')}")
    except Exception as exc:
        print(f"Discover failed: {exc}")
    print(f"\nAlso try: https://data.indy.gov (search 'IMPD' or 'crime')")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest Indianapolis IMPD crime trends by district from ArcGIS."
    )
...
def main() -> None:
    ...
    print(f"Fetching current 12-month Indianapolis crime counts "
    ...
    print(f"\nFetching prior 12-month Indianapolis crime counts "
