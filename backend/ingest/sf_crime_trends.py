"""
backend/ingest/sf_crime_trends.py
task: data-047
lane: data

Ingests San Francisco Police Department crime data and calculates 12-month
...
Source:
  ...
  Verify dataset ID:
    curl "https://data.sfgov.org/api/catalog/v1?q=incident+reports&limit=5"

  Key fields:
    incident_number   — unique ID
    incident_datetime — ISO 8601 datetime string
    police_district   — district name (BAYVIEW, CENTRAL, etc.)
    latitude, longitude — coordinates

Method:
  1. Aggregate crime counts by police_district for the last 12 months.
  2. Aggregate crime counts by police_district for the prior 12 months.
...
Output:
  data/raw/sf_crime_trends.json — district crime trend records

Usage:
  python backend/ingest/sf_crime_trends.py
...
CRIMES_URL = "https://data.sfgov.org/resource/wg3w-h783.json"
SOCRATA_DOMAIN = "data.sfgov.org"

DEFAULT_OUTPUT_PATH = Path("data/raw/sf_crime_trends.json")

# Date field and district field in the SFPD Incident Reports dataset.
# Verify field names:
#   curl "https://data.sfgov.org/resource/wg3w-h783.json?$limit=1" | python -m json.tool
...
def build_trend_records(
    ...
    for district in sorted(all_districts):
        ...
        records.append({
            ...
            "region_id": f"sf_district_{district.lower().replace(' ', '_')}",
            "district_id": district,
            "district_name": f"SF District {district.title()}",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest SFPD crime trends by police district from the Socrata API."
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def main() -> None:
    ...
    print(f"Fetching current 12-month SF crime counts ({_date_str(current_start)} → {_date_str(now)})...")
    try:
    ...
    print(f"\nFetching prior 12-month SF crime counts ({_date_str(prior_start)} → {_date_str(prior_end)})...")
    try:
