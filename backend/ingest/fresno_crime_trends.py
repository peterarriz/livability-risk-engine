"""
backend/ingest/fresno_crime_trends.py
task: data-049
lane: data

Ingests Fresno Police Department crime data and calculates 12-month
...
Output:
  data/raw/fresno_crime_trends.json

Usage:
  python backend/ingest/fresno_crime_trends.py
  python backend/ingest/fresno_crime_trends.py --dry-run
"""
...
FEATURESERVER_URL = (
    ...
    "/City_of_Fresno_Crime_Data_View/FeatureServer/0"
)

DEFAULT_OUTPUT_PATH = Path("data/raw/fresno_crime_trends.json")

DATE_FIELD = "OccurredOn"
GROUP_FIELD = "PD_District"
...
def build_trend_records(
    ...
    for district in sorted(all_districts):
        ...
        records.append({
            ...
            "region_id": f"fresno_district_{slug}",
            "district_id": district,
            "district_name": f"Fresno District {district}",
            "crime_12mo": current_count,
            "crime_prior_12mo": prior_count,
            "crime_trend": trend,
            "crime_trend_pct": trend_pct,
            "latitude": FRESNO_LAT,
            "longitude": FRESNO_LON,
        })
    return records
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest Fresno crime trends by district from ArcGIS FeatureServer."
    )
...
def main() -> None:
    ...
    print(f"Fetching current 12-month Fresno crime counts ({current_start:%Y-%m-%d} → {now:%Y-%m-%d})...")
    try:
    ...
    print(f"\nFetching prior 12-month Fresno crime counts ({prior_start:%Y-%m-%d} → {prior_end:%Y-%m-%d})...")
    try:
