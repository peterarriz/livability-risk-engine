"""
backend/ingest/seattle_crime_trends.py
task: data-044
lane: data

Ingests Seattle SPD crime data and calculates 12-month crime trends by precinct.

Source:
  https://data.seattle.gov/resource/tazs-3rd5.json
  Dataset: SPD Crime Data: 2008-Present (Seattle Police Department)

Method:
  1. Aggregate crime counts by precinct for the last 12 months (current window).
  2. Aggregate crime counts by precinct for the prior 12 months (baseline).
  3. Calculate percent change → crime_trend: INCREASING / DECREASING / STABLE.
  4. Compute approximate precinct centroids from the average lat/lon of incidents.

Output:
  data/raw/seattle_crime_trends.json — precinct crime trend records

Usage:
  python backend/ingest/seattle_crime_trends.py
  python backend/ingest/seattle_crime_trends.py --dry-run
...
CRIMES_URL = "https://data.seattle.gov/resource/tazs-3rd5.json"
...
def build_trend_records(
    ...
    for precinct in sorted(all_precincts):
        ...
        records.append({
            ...
            "region_id": f"seattle_precinct_{precinct}",
            "district_id": precinct,
            "district_name": f"Seattle Precinct {precinct}",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest Seattle SPD crime trends by precinct from the Socrata API."
    )
    parser.add_argument(
        "--output",
        type=Path,
