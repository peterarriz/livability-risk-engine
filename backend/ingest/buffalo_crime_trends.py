"""
backend/ingest/buffalo_crime_trends.py
task: data-056
lane: data

Ingests Buffalo Police Department crime data and calculates 12-month
crime trends by police district.

Source:
  Socrata — data.buffalony.gov
  Dataset: Crime Incidents (d6g9-xbgu)

  Key fields: incident_datetime, police_district, incident_type_primary

Output:
  data/raw/buffalo_crime_trends.json

Usage:
  python backend/ingest/buffalo_crime_trends.py
  python backend/ingest/buffalo_crime_trends.py --dry-run
"""
...
CRIMES_URL = "https://data.buffalony.gov/resource/d6g9-xbgu.json"
SOCRATA_DOMAIN = "data.buffalony.gov"

DEFAULT_OUTPUT_PATH = Path("data/raw/buffalo_crime_trends.json")
...
def build_trend_records(
    ...
    for district in sorted(all_districts):
        ...
        records.append({
            ...
            "region_id": f"buffalo_district_{slug}",
            "district_id": district,
            "district_name": f"Buffalo {district}",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest Buffalo BPD crime trends by district from Socrata."
    )
...
def main() -> None:
    ...
    print(f"Fetching current 12-month Buffalo crime counts ({current_start:%Y-%m-%d} → {now:%Y-%m-%d})...")
    try:
    ...
    print(f"\nFetching prior 12-month Buffalo crime counts ({prior_start:%Y-%m-%d} → {prior_end:%Y-%m-%d})...")
    try:
