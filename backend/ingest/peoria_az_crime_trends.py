...
Usage:
  python backend/ingest/peoria_az_crime_trends.py
...
def build_trend_records(
    ...
    for district in sorted(all_districts):
        ...
        records.append({
            ...
            "latitude": PEORIA_AZ_LAT,
            "longitude": PEORIA_AZ_LON,
        })
    return records
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest Peoria AZ PPD crime trends by district from ArcGIS FeatureServer."
    )
...
def main() -> None:
    ...
    print(f"Fetching current 12-month Peoria AZ crime counts ({current_start:%Y-%m-%d} → {now:%Y-%m-%d})...")
    try:
    ...
    print(f"\nFetching prior 12-month Peoria AZ crime counts ({prior_start:%Y-%m-%d} → {prior_end:%Y-%m-%d})...")
    try:
