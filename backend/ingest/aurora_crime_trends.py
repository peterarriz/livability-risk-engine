"""
backend/ingest/aurora_crime_trends.py
task: data-057
lane: data

Ingests Aurora CO Police Department crime data and calculates 12-month crime
...
Output:
  data/raw/aurora_crime_trends.json

Usage:
  python backend/ingest/aurora_crime_trends.py
  python backend/ingest/aurora_crime_trends.py --dry-run
"""
...
def build_trend_records(
    ...
    for district in sorted(all_districts):
        ...
        records.append({
            ...
            "region_id": f"aurora_district_{district}",
            "district_id": district,
            "district_name": f"Aurora District {district}",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest Aurora CO APD crime trends by district from ArcGIS."
    )
...
def main() -> None:
    ...
    print("Fetching current 12-month Aurora crime counts...")
    try:
        current_data = fetch_crime_counts(current_start, now)
    except Exception as exc:
        print(f"ERROR: failed to fetch current Aurora crime counts — {exc}", file=sys.stderr)
        sys.exit(1)
    print(f"  {len(current_data)} districts with current crime data.")

    print("Fetching prior 12-month Aurora crime counts...")
    try:
        prior_data = fetch_crime_counts(prior_start, prior_end)
    except Exception as exc:
        print(f"ERROR: failed to fetch prior Aurora crime counts — {exc}", file=sys.stderr)
        sys.exit(1)
