"""
backend/ingest/durham_crime_trends.py
task: data-058
lane: data

Ingests Durham Police Department (DPD) crime data and calculates
12-month crime trends by district/division.
...
Usage:
  python backend/ingest/durham_crime_trends.py
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest Durham DPD crime trends by district from ArcGIS FeatureServer."
    )
...
def main() -> None:
    ...
    print(f"Fetching current 12-month Durham crime counts ({current_start:%Y-%m-%d} → {now:%Y-%m-%d})...")
    try:
    ...
    print(f"\nFetching prior 12-month Durham crime counts ({prior_start:%Y-%m-%d} → {prior_end:%Y-%m-%d})...")
    try:
