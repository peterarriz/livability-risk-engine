...
crime trends by division via ArcGIS REST API.

Source:
  Dallas Open Data — Dallas Police Department Crime Incidents:
  https://www.dallasopendata.com
  https://services.arcgis.com/K1vmv3C6RR68oGEo/arcgis/rest/services
      /DPD_CrimeIncidents/FeatureServer/0
  ...
  Verify endpoint and field names:
    curl "https://services.arcgis.com/K1vmv3C6RR68oGEo/arcgis/rest/services/DPD_CrimeIncidents/FeatureServer/0/query?where=1%3D1&outFields=*&resultRecordCount=1&f=json"

  Key fields (MUST VERIFY):
    IncidentDate  — date field (may be "Incident_Date" or "date1")
    Division      — police division (may be "Beat", "Sector", or "ReportingArea")
    Latitude      — double
    Longitude     — double
    ObjectId      — used for count()

Method:
  1. Aggregate crime counts + avg lat/lon by division for the last 12 months
     via outStatistics.
  2. Aggregate crime counts + avg lat/lon by division for the prior 12 months.
  3. Calculate percent change → crime_trend: INCREASING / DECREASING / STABLE.

Output:
  data/raw/dallas_crime_trends.json — division crime trend records
...
ARCGIS_BASE_URL = (
    "https://services.arcgis.com/K1vmv3C6RR68oGEo/arcgis/rest/services"
    "/DPD_CrimeIncidents/FeatureServer/0/query"
)

DEFAULT_OUTPUT_PATH = Path("data/raw/dallas_crime_trends.json")

DATE_FIELD = "IncidentDate"   # MUST VERIFY — may be "Incident_Date" or "date1"
DISTRICT_FIELD = "Division"   # MUST VERIFY — may be "Beat", "Sector", or "ReportingArea"
...
def fetch_crime_counts_with_centroids(
    ...
    """
    Fetch total crime counts and average centroids per division for a date range.
...
def build_trend_records(
    ...
    for district in sorted(all_districts):
        ...
        records.append({
            ...
            "region_id": f"dallas_division_{district}",
            "district_id": district,
            "district_name": f"Dallas Division {district}",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest Dallas PD crime trends by division from ArcGIS REST API."
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def main() -> None:
    ...
    print(f"Fetching current 12-month Dallas crime counts ({_timestamp_str(current_start)} → {_timestamp_str(now)})...")
    try:
    ...
    print(f"\nFetching prior 12-month Dallas crime counts ({_timestamp_str(prior_start)} → {_timestamp_str(prior_end)})...")
    try:
