...
- FastAPI `/score` endpoint returning score + contributing factors
- Minimal Next.js frontend that calls `/score` and displays score + list of nearby projects
...
1. Given a Chicago address, the API returns a JSON response in <500ms containing `score` (0–100) and at least one contributing project.
...
3. Frontend displays a score and list of active projects for the input address and matches the API response.
