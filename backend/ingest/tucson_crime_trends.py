"""
backend/ingest/tucson_crime_trends.py
task: data-049
lane: data

Ingests Tucson Police Department crime data and calculates 12-month
crime trends by division.

Source:
  ArcGIS FeatureServer — Tucson Police Reported Crimes
  https://services3.arcgis.com/9coHY2fvuFjG9HQX/arcgis/rest/services/Tucson_Police_Reported_Crimes/FeatureServer/8

  Key fields: DateOccurred, Division, Ward, UCRDescription, Offense

  Note: This is a Table layer (no point geometry).

Output:
  data/raw/tucson_crime_trends.json

Usage:
  python backend/ingest/tucson_crime_trends.py
  python backend/ingest/tucson_crime_trends.py --dry-run
"""
...
FEATURESERVER_URL = (
    ...
    "/Tucson_Police_Reported_Crimes/FeatureServer/8"
)

DEFAULT_OUTPUT_PATH = Path("data/raw/tucson_crime_trends.json")
...
def fetch_crime_counts(
    ...
    out_statistics = json.dumps([
        {"statisticType": "count", "onStatisticField": "ESRI_OID",
...
def build_trend_records(
    ...
    for division in sorted(all_divisions):
        ...
        records.append({
            ...
            "region_id": f"tucson_division_{slug}",
            "district_id": division,
            "district_name": f"Tucson Division {division}",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest Tucson TPD crime trends by division from ArcGIS FeatureServer."
    )
...
def main() -> None:
    ...
    print(f"Fetching current 12-month Tucson crime counts ({current_start:%Y-%m-%d} → {now:%Y-%m-%d})...")
    try:
    ...
    print(f"\nFetching prior 12-month Tucson crime counts ({prior_start:%Y-%m-%d} → {prior_end:%Y-%m-%d})...")
    try:
