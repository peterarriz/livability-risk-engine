...
Ingests McKinney Police Department crime data and calculates 12-month
crime trends by beat via ArcGIS REST API.

Source:
  McKinney Open Data — MPD Crime Incidents:
  https://mckinneytexas.org/opendata or https://mckinney.opendata.arcgis.com
  https://services.arcgis.com/5VpNVlUxHMX5rB9c/arcgis/rest/services
  ...
  Verify service listing:
    curl -s "https://services.arcgis.com/5VpNVlUxHMX5rB9c/arcgis/rest/services?f=json" \
      | python3 -c "import sys,json; [print(s['name']) for s in json.load(sys.stdin).get('services',[])]"

  Verify endpoint and field names:
    curl "https://services.arcgis.com/5VpNVlUxHMX5rB9c/arcgis/rest/services/MPD_Crime_Incidents/FeatureServer/0/query?where=1%3D1&outFields=*&resultRecordCount=1&f=json"
...
Method:
  1. Aggregate crime counts + avg lat/lon by beat for the last 12 months
     via outStatistics.
  2. Aggregate crime counts + avg lat/lon by beat for the prior 12 months.
  3. Calculate percent change → crime_trend: INCREASING / DECREASING / STABLE.

Output:
  data/raw/mckinney_tx_crime_trends.json — beat crime trend records

Usage:
  python backend/ingest/mckinney_tx_crime_trends.py
  python backend/ingest/mckinney_tx_crime_trends.py --dry-run
"""
...
DEFAULT_OUTPUT_PATH = Path("data/raw/mckinney_tx_crime_trends.json")

DATE_FIELD = "IncidentDate"   # MUST VERIFY
DISTRICT_FIELD = "Beat"       # MUST VERIFY
...
MCKINNEY_LAT = 33.1972
MCKINNEY_LON = -96.6397
...
def fetch_crime_counts_with_centroids(
    ...
    """
    Fetch total crime counts and average centroids per beat for a date range.
...
def build_trend_records(
    ...
    for district in sorted(all_districts):
        ...
        records.append({
            "region_type": "beat",
            "region_id": f"mckinney_beat_{district}",
            "district_id": district,
            "district_name": f"McKinney Beat {district}",
...
def write_staging_file(records: list[dict], output_path: Path) -> None:
    ...
    staging = {
        "source": "mckinney_tx_crime_trends",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest McKinney PD crime trends by beat from ArcGIS REST API."
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def main() -> None:
    ...
    print(f"Fetching current 12-month McKinney crime counts ({_timestamp_str(current_start)} → {_timestamp_str(now)})...")
    try:
    ...
    print(f"\nFetching prior 12-month McKinney crime counts ({_timestamp_str(prior_start)} → {_timestamp_str(prior_end)})...")
    try:
