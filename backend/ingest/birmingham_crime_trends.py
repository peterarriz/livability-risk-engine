...
Ingests Birmingham Police Department crime data and calculates 12-month
crime trends by precinct via ArcGIS REST API.

Source:
  Birmingham, AL ArcGIS Hub — BPD Crime Incidents:
  https://birminghamal.maps.arcgis.com
  ...
  Verify endpoint and field names:
    curl "https://services6.arcgis.com/iFT94KHJdBf1glgr/arcgis/rest/services/BPD_Crime_Incidents/FeatureServer/0/query?where=1%3D1&outFields=*&resultRecordCount=1&f=json"

  Key fields (MUST VERIFY):
    IncidentDate  — date field
    Precinct      — police precinct (may be "Division" or "Beat")
    Latitude      — double
    Longitude     — double
...
Output:
  data/raw/birmingham_crime_trends.json — precinct crime trend records

Usage:
  python backend/ingest/birmingham_crime_trends.py
  python backend/ingest/birmingham_crime_trends.py --dry-run
"""
...
ARCGIS_BASE_URL = (
    "https://services6.arcgis.com/iFT94KHJdBf1glgr/arcgis/rest/services"
    "/BPD_Crime_Incidents/FeatureServer/0/query"
)

DEFAULT_OUTPUT_PATH = Path("data/raw/birmingham_crime_trends.json")

DATE_FIELD = "IncidentDate"   # MUST VERIFY
DISTRICT_FIELD = "Precinct"   # MUST VERIFY — may be "Division" or "Beat"
...
def build_trend_records(
    ...
    for district in sorted(all_districts):
        ...
        records.append({
            ...
            "region_id": f"birmingham_precinct_{district}",
            "district_id": district,
            "district_name": f"Birmingham Precinct {district}",
...
def write_staging_file(records: list[dict], output_path: Path) -> None:
    ...
    staging = {
        "source": "birmingham_crime_trends",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest Birmingham PD crime trends by precinct from ArcGIS REST API."
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def main() -> None:
    ...
    print(f"Fetching current 12-month Birmingham crime counts ({_timestamp_str(current_start)} → {_timestamp_str(now)})...")
    try:
    ...
    print(f"\nFetching prior 12-month Birmingham crime counts ({_timestamp_str(prior_start)} → {_timestamp_str(prior_end)})...")
    try:
