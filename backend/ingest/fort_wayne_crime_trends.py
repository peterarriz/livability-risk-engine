...
Ingests Fort Wayne Police Department (FWPD) crime data and calculates
12-month crime trends by district.

Source:
  ArcGIS FeatureServer — data.fortwayne.com (ArcGIS Hub)
  Portal: https://data.fortwayne.com

  MUST VERIFY service URL before production:
    1. Visit https://data.fortwayne.com
  ...
  Current estimate (MUST VERIFY):
    Org ID:       8Wez4BJD3neYYnDt  (estimate — must verify)
    Service name: FWPD_Crime_Incidents  (estimate — must verify)
    Date field:   DateOccurred  (MUST VERIFY)
...
Output:
  data/raw/fort_wayne_crime_trends.json
...
# MUST VERIFY: visit https://data.fortwayne.com and find crime/police incidents dataset.
...
FEATURESERVER_URL = (
    ...
    "/FWPD_Crime_Incidents/FeatureServer/0"
)
...
def build_trend_records(
    ...
    for district in sorted(all_districts):
        ...
        records.append({
            ...
            "region_id": f"fort_wayne_district_{slug}",
            "district_id": district,
            "district_name": f"Fort Wayne District {district}",
            "crime_12mo": current_count,
            "crime_prior_12mo": prior_count,
            "crime_trend": trend,
            "crime_trend_pct": trend_pct,
            "latitude": FORT_WAYNE_LAT,
            "longitude": FORT_WAYNE_LON,
        })
    return records
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest Fort Wayne FWPD crime trends by district from ArcGIS FeatureServer."
    )
...
def main() -> None:
    ...
    print(f"Fort Wayne crime trends ingest — source: {FEATURESERVER_URL}")
    print("NOTE: Service URL and field names are MUST VERIFY estimates.")
    print("      Visit https://data.fortwayne.com to find the correct FeatureServer URL.")
