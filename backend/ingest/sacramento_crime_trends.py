"""
backend/ingest/sacramento_crime_trends.py
task: data-049
lane: data

Ingests Sacramento Police Department crime data and calculates 12-month
crime trends by police district.

Source:
  ArcGIS FeatureServer — Sacramento Police Crime 3 Years
  https://services5.arcgis.com/54falWtcpty3V47Z/arcgis/rest/services/Police_Crime_3Years/FeatureServer/0

  Key fields: Occurrence_Date_UTC, Police_District, Beat, Offense_Category

Output:
  data/raw/sacramento_crime_trends.json

Usage:
  python backend/ingest/sacramento_crime_trends.py
  python backend/ingest/sacramento_crime_trends.py --dry-run
"""
...
FEATURESERVER_URL = (
    ...
    "/Police_Crime_3Years/FeatureServer/0"
)

DEFAULT_OUTPUT_PATH = Path("data/raw/sacramento_crime_trends.json")

DATE_FIELD = "Occurrence_Date_UTC"
GROUP_FIELD = "Police_District"

SACRAMENTO_LAT = 38.5816
SACRAMENTO_LON = -121.4944
...
def build_trend_records(
    ...
    for district in sorted(all_districts):
        ...
        records.append({
            ...
            "region_id": f"sacramento_district_{slug}",
            "district_id": district,
            "district_name": f"Sacramento District {district}",
            "crime_12mo": current_count,
            "crime_prior_12mo": prior_count,
            "crime_trend": trend,
            "crime_trend_pct": trend_pct,
            "latitude": SACRAMENTO_LAT,
            "longitude": SACRAMENTO_LON,
        })
    return records
...
def write_staging_file(records: list[dict], output_path: Path) -> None:
    ...
    staging = {
        "source": "sacramento_crime_trends",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest Sacramento SPD crime trends by district from ArcGIS FeatureServer."
    )
...
def main() -> None:
    ...
    print(f"Fetching current 12-month Sacramento crime counts ({current_start:%Y-%m-%d} → {now:%Y-%m-%d})...")
    try:
    ...
    print(f"\nFetching prior 12-month Sacramento crime counts ({prior_start:%Y-%m-%d} → {prior_end:%Y-%m-%d})...")
    try:
