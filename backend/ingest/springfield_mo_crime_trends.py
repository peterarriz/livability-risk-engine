...
Ingests Springfield (MO) Police Department crime data and calculates
12-month crime trends by district/zone.

Source:
  ArcGIS FeatureServer — data.springfieldmo.gov (City of Springfield Open Data)
  Portal: https://data.springfieldmo.gov
  ...
  To verify:
    ...
    Or visit: https://data.springfieldmo.gov and search "crime" or "police"
  ...
  Key fields (MUST VERIFY field names via --dry-run):
    ...
    Zone          — patrol zone/district (MUST VERIFY)

Output:
  data/raw/springfield_mo_crime_trends.json
...
# MUST VERIFY — visit https://data.springfieldmo.gov and search "crime incidents"
FEATURESERVER_URL = (
    "https://services6.arcgis.com/bdLPgVQpKkp3xrEm/arcgis/rest/services"
    "/SPD_Crime_Incidents/FeatureServer/0"
)

DEFAULT_OUTPUT_PATH = Path("data/raw/springfield_mo_crime_trends.json")

# MUST VERIFY field names via --dry-run
DATE_FIELD = "IncidentDate"
GROUP_FIELD = "Zone"

SPRINGFIELD_MO_LAT = 37.2153
SPRINGFIELD_MO_LON = -93.2982
...
def build_trend_records(
    ...
    for zone in sorted(all_zones):
        ...
        records.append({
            "region_type": "zone",
            "region_id": f"springfield_mo_zone_{slug}",
            "district_id": zone,
            "district_name": f"Springfield MO Zone {zone}",
            "crime_12mo": current_count,
            "crime_prior_12mo": prior_count,
            "crime_trend": trend,
            "crime_trend_pct": trend_pct,
            "latitude": SPRINGFIELD_MO_LAT,
            "longitude": SPRINGFIELD_MO_LON,
        })
    return records
...
def write_staging_file(records: list[dict], output_path: Path) -> None:
    ...
    staging = {
        "source": "springfield_mo_crime_trends",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest Springfield MO SPD crime trends by zone from ArcGIS FeatureServer."
    )
...
def main() -> None:
    ...
    print(f"Springfield MO crime trends ingest — source: {FEATURESERVER_URL}")
    print("NOTE: Service URL and field names are MUST VERIFY estimates.")
    print("Verify via: https://data.springfieldmo.gov (search 'crime')")
