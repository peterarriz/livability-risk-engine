"""
backend/ingest/philadelphia_crime_trends.py
task: data-056
lane: data

Ingests Philadelphia Police Department crime data and calculates 12-month
crime trends by police district.

Source:
  Carto SQL API — Philadelphia crime incidents (Part I + Part II)
  https://phl.carto.com/api/v2/sql

  Key fields: dispatch_date_time, dc_dist, text_general_code

Output:
  data/raw/philadelphia_crime_trends.json

Usage:
  python backend/ingest/philadelphia_crime_trends.py
  python backend/ingest/philadelphia_crime_trends.py --dry-run
"""
...
DEFAULT_OUTPUT_PATH = Path("data/raw/philadelphia_crime_trends.json")
...
def fetch_crime_counts(
    ...
    resp = requests.get(CARTO_SQL_URL, params={"q": sql}, timeout=60)
    ...
    for row in payload.get("rows", []):
        ...
        if not district:
            continue
        results[district] = int(row.get("crime_count", 0))
    return results
...
def build_trend_records(
    ...
    for district in sorted(all_districts):
        ...
        records.append({
            ...
            "region_id": f"philadelphia_district_{slug}",
            "district_id": district,
            "district_name": f"Philadelphia District {district}",
            "crime_12mo": current_count,
            "crime_prior_12mo": prior_count,
            "crime_trend": trend,
            "crime_trend_pct": trend_pct,
            "latitude": PHILADELPHIA_LAT,
            "longitude": PHILADELPHIA_LON,
        })
    return records
...
def write_staging_file(records: list[dict], output_path: Path) -> None:
    ...
    staging = {
        "source": "philadelphia_crime_trends",
        "source_url": CARTO_SQL_URL,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest Philadelphia crime trends by district from Carto SQL API."
    )
...
def main() -> None:
    ...
    print(f"Fetching current 12-month Philadelphia crime counts ({current_start:%Y-%m-%d} → {now:%Y-%m-%d})...")
    try:
    ...
    print(f"\nFetching prior 12-month Philadelphia crime counts ({prior_start:%Y-%m-%d} → {prior_end:%Y-%m-%d})...")
    try:
