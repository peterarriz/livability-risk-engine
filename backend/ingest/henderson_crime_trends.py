...
12-month crime trends by area/bureau.
...
Source:
  ArcGIS FeatureServer — Henderson Open Data (ArcGIS Hub)
  Portal: https://hendersonnv.gov/opendata  (or henderson-open-data.hub.arcgis.com)
  Service: HPD Crime Incidents (MUST VERIFY service URL)
  Verify: Visit the Henderson open data portal, search "crime incidents",
  ...
  Key fields (MUST VERIFY field names via --dry-run):
    ...
    Area or Bureau             — geographic grouping
...
# MUST VERIFY: visit Henderson NV open data portal → search "crime" → API tab
FEATURESERVER_URL = (
    "https://services.arcgis.com/pGfbNXXgj2WN9j5V/arcgis/rest/services"
    "/HPD_Crime_Incidents/FeatureServer/0"
)
...
GROUP_FIELD = "Area"         # MUST VERIFY — may be "Bureau" or "Division"
...
def build_trend_records(
    ...
    for area in sorted(all_areas):
        ...
        records.append({
            "region_type": "area",
            "region_id": f"henderson_area_{slug}",
            "district_id": area,
            "district_name": f"Henderson Area {area}",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest Henderson HPD crime trends by area from ArcGIS FeatureServer."
    )
