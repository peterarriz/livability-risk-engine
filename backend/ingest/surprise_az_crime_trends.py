...
Usage:
  python backend/ingest/surprise_az_crime_trends.py
...
def build_trend_records(
    ...
    for district in sorted(all_districts):
        ...
        records.append({
            ...
            "region_id": f"surprise_az_district_{slug}",
            "district_id": district,
            "district_name": f"Surprise AZ {district}",
            "crime_12mo": current_count,
            "crime_prior_12mo": prior_count,
            "crime_trend": trend,
            "crime_trend_pct": trend_pct,
            "latitude": SURPRISE_AZ_LAT,
            "longitude": SURPRISE_AZ_LON,
        })
    return records
...
def write_staging_file(records: list[dict], output_path: Path) -> None:
    ...
    staging = {
        "source": "surprise_az_crime_trends",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest Surprise AZ SPD crime trends by district from ArcGIS FeatureServer."
    )
...
def main() -> None:
    ...
    print(f"Fetching current 12-month Surprise AZ crime counts ({current_start:%Y-%m-%d} → {now:%Y-%m-%d})...")
    try:
    ...
    print(f"\nFetching prior 12-month Surprise AZ crime counts ({prior_start:%Y-%m-%d} → {prior_end:%Y-%m-%d})...")
    try:
