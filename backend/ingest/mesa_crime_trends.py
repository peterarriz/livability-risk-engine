...
Source:
  ArcGIS Hub — data-mesagis.opendata.arcgis.com (Mesa GIS open data)
  FeatureServer URL (MUST VERIFY):
    https://services2.arcgis.com/T3Rrfm3Dqq8Eepqn/arcgis/rest/services/
    Mesa_PD_Crime/FeatureServer/0

  Verify: python backend/ingest/mesa_crime_trends.py --dry-run
  Or check: https://data-mesagis.opendata.arcgis.com (search "crime" or "police")
  Or: curl "https://hub.arcgis.com/api/v3/search?q=crime+Mesa+AZ+police&page[size]=5"

  Note: Mesa is a separate city from Phoenix/Maricopa County. The Phoenix
  ArcGIS datasets (phoenix_crime_trends.py) do NOT cover Mesa.
...
# MUST VERIFY service URL via: https://data-mesagis.opendata.arcgis.com
FEATURESERVER_URL = (
    "https://services2.arcgis.com/T3Rrfm3Dqq8Eepqn/arcgis/rest/services"
    "/Mesa_PD_Crime/FeatureServer/0"
)
...
def build_trend_records(
    ...
    for district in sorted(all_districts):
        ...
        records.append({
            ...
            "region_id": f"mesa_district_{district}",
            "district_id": district,
            "district_name": f"Mesa District {district}",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest Mesa PD crime trends by district from ArcGIS."
    )
...
def main() -> None:
    ...
    print("Fetching current 12-month Mesa crime counts...")
    try:
        current_data = fetch_crime_counts(current_start, now)
    except Exception as exc:
        print(f"ERROR: failed to fetch current Mesa crime counts — {exc}", file=sys.stderr)
        sys.exit(1)
    print(f"  {len(current_data)} districts with current crime data.")

    print("Fetching prior 12-month Mesa crime counts...")
    try:
        prior_data = fetch_crime_counts(prior_start, prior_end)
    except Exception as exc:
        print(f"ERROR: failed to fetch prior Mesa crime counts — {exc}", file=sys.stderr)
        sys.exit(1)
