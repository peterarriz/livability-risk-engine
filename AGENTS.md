...
## Repo purpose
Livability Risk Engine is a Chicago-only MVP for producing a near-term construction disruption risk score for a single address. The MVP output is a JSON API response with a `score`, confidence, nearby projects, and a short explanation. Work in this repo should improve delivery of that MVP only.
