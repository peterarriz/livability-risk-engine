...
Ingests Frisco Police Department crime data and calculates 12-month
crime trends by beat via ArcGIS REST API.

Source:
  Frisco Open Data — FPD Crime Incidents:
  https://friscotexas.gov/opendata or https://frisco.opendata.arcgis.com
  https://services.arcgis.com/GE4Z4z1cnF58LL3C/arcgis/rest/services
      /FPD_Crime_Incidents/FeatureServer/0
  ...
  Verify service listing:
    curl -s "https://services.arcgis.com/GE4Z4z1cnF58LL3C/arcgis/rest/services?f=json" \
      | python3 -c "import sys,json; [print(s['name']) for s in json.load(sys.stdin).get('services',[])]"

  Verify endpoint and field names:
    curl "https://services.arcgis.com/GE4Z4z1cnF58LL3C/arcgis/rest/services/FPD_Crime_Incidents/FeatureServer/0/query?where=1%3D1&outFields=*&resultRecordCount=1&f=json"
...
Output:
  data/raw/frisco_tx_crime_trends.json — beat crime trend records
...
ARCGIS_BASE_URL = (
    "https://services.arcgis.com/GE4Z4z1cnF58LL3C/arcgis/rest/services"
    "/FPD_Crime_Incidents/FeatureServer/0/query"
)
...
def build_trend_records(
    ...
    for district in sorted(all_districts):
        ...
        records.append({
            "region_type": "beat",
            "region_id": f"frisco_beat_{district}",
            "district_id": district,
            "district_name": f"Frisco Beat {district}",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest Frisco PD crime trends by beat from ArcGIS REST API."
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def main() -> None:
    ...
    print(f"Fetching current 12-month Frisco crime counts ({_timestamp_str(current_start)} → {_timestamp_str(now)})...")
    try:
    ...
    print(f"\nFetching prior 12-month Frisco crime counts ({_timestamp_str(prior_start)} → {_timestamp_str(prior_end)})...")
    try:
