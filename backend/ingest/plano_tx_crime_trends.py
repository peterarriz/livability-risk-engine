...
Ingests Plano Police Department crime data and calculates 12-month
crime trends by sector via ArcGIS REST API.

Source:
  Plano Open Data — PPD Crime Incidents:
  https://data.plano.gov
  https://services.arcgis.com/J6sY5RXbVdFl1rTf/arcgis/rest/services
      /PPD_Crime_Incidents/FeatureServer/0
  ...
  Verify endpoint and field names:
    curl "https://services.arcgis.com/J6sY5RXbVdFl1rTf/arcgis/rest/services/PPD_Crime_Incidents/FeatureServer/0/query?where=1%3D1&outFields=*&resultRecordCount=1&f=json"
...
Output:
  data/raw/plano_tx_crime_trends.json — sector crime trend records
...
def build_trend_records(
    ...
    for district in sorted(all_districts):
        ...
        records.append({
            "region_type": "sector",
            "region_id": f"plano_sector_{district}",
            "district_id": district,
            "district_name": f"Plano Sector {district}",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest Plano PD crime trends by sector from ArcGIS REST API."
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def main() -> None:
    ...
    print(f"Fetching current 12-month Plano crime counts ({_timestamp_str(current_start)} → {_timestamp_str(now)})...")
    try:
    ...
    print(f"\nFetching prior 12-month Plano crime counts ({_timestamp_str(prior_start)} → {_timestamp_str(prior_end)})...")
    try:
