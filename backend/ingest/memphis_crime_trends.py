"""
backend/ingest/memphis_crime_trends.py
task: data-049
lane: data

Ingests Memphis Police Department crime data and calculates 12-month
crime trends by precinct.

Source:
  ArcGIS FeatureServer — MPD Public Safety Incidents
  https://services2.arcgis.com/saWmpKJIUAjyyNVc/arcgis/rest/services/MPD_Public_Safety_Incidents/FeatureServer/0

  Key fields: Offense_Datetime, Precinct, UCR_Category, Latitude, Longitude

Output:
  data/raw/memphis_crime_trends.json

Usage:
  python backend/ingest/memphis_crime_trends.py
  python backend/ingest/memphis_crime_trends.py --dry-run
"""
...
DEFAULT_OUTPUT_PATH = Path("data/raw/memphis_crime_trends.json")

DATE_FIELD = "Offense_Datetime"
...
def fetch_crime_counts(
    ...
    for feature in payload.get("features", []):
        ...
        if not precinct:
            continue
        results[precinct] = results.get(precinct, 0) + int(attrs.get("crime_count", 0))
    return results
...
def build_trend_records(
    ...
    for precinct in sorted(all_precincts):
        ...
        records.append({
            ...
            "region_id": f"memphis_precinct_{slug}",
            "district_id": precinct,
            "district_name": f"Memphis Precinct {precinct}",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest Memphis MPD crime trends by precinct from ArcGIS FeatureServer."
    )
...
def main() -> None:
    ...
    print(f"Fetching current 12-month Memphis crime counts ({current_start:%Y-%m-%d} → {now:%Y-%m-%d})...")
    try:
    ...
    print(f"\nFetching prior 12-month Memphis crime counts ({prior_start:%Y-%m-%d} → {prior_end:%Y-%m-%d})...")
    try:
