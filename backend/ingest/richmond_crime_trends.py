"""
backend/ingest/richmond_crime_trends.py
task: data-057
lane: data

Ingests Richmond Police Department crime data and calculates 12-month crime
trends by precinct/zone.
...
Source:
  ...
  Verify: python backend/ingest/richmond_crime_trends.py --dry-run
...
Output:
  data/raw/richmond_crime_trends.json

Usage:
  python backend/ingest/richmond_crime_trends.py
  python backend/ingest/richmond_crime_trends.py --dry-run
"""
...
def build_trend_records(
    ...
    for zone in sorted(all_zones):
        ...
        records.append({
            ...
            "region_id": f"richmond_precinct_{zone}",
            "district_id": zone,
            "district_name": f"Richmond Precinct {zone}",
            "crime_12mo": current_count,
            "crime_prior_12mo": prior_count,
            "crime_trend": trend,
            "crime_trend_pct": trend_pct,
            "latitude": RICHMOND_LAT,
            "longitude": RICHMOND_LON,
        })
    return records
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest Richmond RPD crime trends by precinct from ArcGIS."
    )
...
def main() -> None:
    ...
    print("Fetching current 12-month Richmond crime counts...")
    try:
        current_data = fetch_crime_counts(current_start, now)
    except Exception as exc:
        print(f"ERROR: failed to fetch current Richmond crime counts — {exc}", file=sys.stderr)
        sys.exit(1)
    print(f"  {len(current_data)} precincts with current crime data.")

    print("Fetching prior 12-month Richmond crime counts...")
    try:
        prior_data = fetch_crime_counts(prior_start, prior_end)
    except Exception as exc:
        print(f"ERROR: failed to fetch prior Richmond crime counts — {exc}", file=sys.stderr)
        sys.exit(1)
