"""
backend/ingest/detroit_crime_trends.py
task: data-056
lane: data

Ingests Detroit Police Department crime data and calculates 12-month
crime trends by precinct.

Source:
  ArcGIS FeatureServer — RMS Crime Incidents
  https://services2.arcgis.com/qvkbeam7Wirps6zC/arcgis/rest/services/RMS_Crime_Incidents/FeatureServer/0

  Key fields: incident_occurred_at, police_precinct, offense_category

Output:
  data/raw/detroit_crime_trends.json

Usage:
  python backend/ingest/detroit_crime_trends.py
  python backend/ingest/detroit_crime_trends.py --dry-run
"""
...
FEATURESERVER_URL = (
    ...
    "/RMS_Crime_Incidents/FeatureServer/0"
)

DEFAULT_OUTPUT_PATH = Path("data/raw/detroit_crime_trends.json")

DATE_FIELD = "incident_occurred_at"
GROUP_FIELD = "police_precinct"
...
def build_trend_records(
    ...
    for precinct in sorted(all_precincts):
        ...
        records.append({
            ...
            "region_id": f"detroit_precinct_{slug}",
            "district_id": precinct,
            "district_name": f"Detroit Precinct {precinct}",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingest Detroit DPD crime trends by precinct from ArcGIS FeatureServer."
    )
...
def main() -> None:
    ...
    print(f"Fetching current 12-month Detroit crime counts ({current_start:%Y-%m-%d} → {now:%Y-%m-%d})...")
    try:
    ...
    print(f"\nFetching prior 12-month Detroit crime counts ({prior_start:%Y-%m-%d} → {prior_end:%Y-%m-%d})...")
    try:
